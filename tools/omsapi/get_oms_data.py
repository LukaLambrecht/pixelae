...
def get_oms_data(omsapi, api_endpoint,
                 runnb=None, fillnb=None,
                 extrafilters=[], extraargs={},
                 sort=None, attributes=[],
                 limit_entries=1000):
    """
...
def get_oms_data_iterative(omsapi, api_endpoint, runrange, attributes, **kwargs):
    """
    Same as get_oms_data but loop iteratively over a range of runs
    """
    # note: the maximum allowed limit in OMS calls seems to be 100k lumisections;
    #       need to split run range in parts if the number of lumisections is higher.
    limit_entries = 100000 # hard limit in OMS API
    # get lumisection numbers (only to find how many there are)
    ls_info_probe = get_oms_data( omsapi, 'lumisections', runrange,
                    attributes=['lumisection_number'], limit_entries=limit_entries )
    # if smaller than the hard limit, just call get_oms_data
    if len(get_oms_response_attribute(ls_info_probe,'lumisection_number')) < limit_entries:
        ls_info_raw = get_oms_data(omsapi, api_endpoint, runrange,
                                   attributes=attributes, limit_entries=limit_entries, **kwargs)
        ls_info = {attribute: get_oms_response_attribute(ls_info_raw, attribute) for attribute in attributes}
        return ls_info
    # else split the run range in half and run recursively on both halves
    else:
        midrange = int((runrange[0]+runrange[1])/2)
        firstrange = (runrange[0], midrange)
        ls_info_1 = get_oms_data_iterative(omsapi, api_endpoint, firstrange, attributes)
        secondrange = (midrange+1, runrange[1])
        ls_info_2 = get_oms_data_iterative(omsapi, api_endpoint, secondrange, attributes)
...
def get_oms_response_attributes( omsresponse ):
    ...
    if( not 'data' in omsresponse.keys()
        or len(omsresponse['data'])==0
        or not 'attributes' in omsresponse['data'][0].keys() ):
