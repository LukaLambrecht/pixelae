...
def residual_block_enc(x, filter_number, kernel_size, strides=1, endactivation=None, **kwargs):
    ...
    if strides>1: x = Conv2D(filter_number, 1, strides=strides, **kwargs)(x)
    out = Add()([x, y])
...
def residual_block_dec(x, filter_number, kernel_size, strides=1, endactivation=None, **kwargs):
    ...
    if strides>1: x = Conv2DTranspose(filter_number, 1, strides=strides, **kwargs)(x)
    out = Add()([x, y])
    if endactivation=='relu': out = ReLU()(out)
    return out
...
def model_mwe( input_shape ):
    # minimal working example, november 15 2023
    input_layer = Input(shape=input_shape)
